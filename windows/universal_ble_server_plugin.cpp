...
void UniversalBleServerPlugin::RegisterWithRegistrar(
  ...
  plugin->read_channel_ =
      std::make_unique<flutter::EventChannel<flutter::EncodableValue>>(
          registrar->messenger(), "universal_ble_server/onRead",
          &flutter::StandardMethodCodec::GetInstance());
  auto read_handler = std::make_unique<
      flutter::StreamHandlerFunctions<flutter::EncodableValue>>(
      [plugin_pointer](
          const flutter::EncodableValue *,
          std::unique_ptr<flutter::EventSink<flutter::EncodableValue>> &&sink) {
...
void UniversalBleServerPlugin::HandleMethodCall(
  ...
  const auto &args = method_call.arguments() ? *method_call.arguments()
                                             : flutter::EncodableValue();
  if (method_call.method_name().compare("startServer") == 0) {
    if (const auto *map = std::get_if<flutter::EncodableMap>(&args)) {
      auto service_uuid = winrt::guid(std::get<std::string>(
          (*map).at(flutter::EncodableValue("serviceUuid"))));
      ...
      if (auto *list = std::get_if<flutter::EncodableList>(
              &(*map).at(flutter::EncodableValue("characteristics")))) {
        for (auto &item : *list) {
          auto charMap = std::get<flutter::EncodableMap>(item);
          auto cuuid = winrt::guid(
              std::get<std::string>(charMap[flutter::EncodableValue("uuid")]));
          auto propsList = std::get<flutter::EncodableList>(
              charMap[flutter::EncodableValue("properties")]);
          ...
          auto valueB64 =
              std::get<std::string>(charMap[flutter::EncodableValue("value")]);
          if (!valueB64.empty()) {
            auto buffer = Windows::Security::Cryptography::CryptographicBuffer::
                DecodeFromBase64String(winrt::to_hstring(valueB64));
            params.StaticValue(buffer);
          }
          auto createRes =
              service.CreateCharacteristicAsync(cuuid, params).get();
          ...
          ch.WriteRequested([&](auto &&, GattWriteRequestedEventArgs args) {
            ...
            if (write_sink_) {
              flutter::EncodableMap evt{
                  {flutter::EncodableValue("uuid"),
                   flutter::EncodableValue(winrt::to_string(cuuid))},
    ...
    if (const auto *map = std::get_if<flutter::EncodableMap>(&args)) {
      auto cuuid = winrt::guid(std::get<std::string>(
          map->at(flutter::EncodableValue("characteristicUuid"))));
      auto valueStr =
          std::get<std::string>(map->at(flutter::EncodableValue("value")));
      if (characteristics.count(cuuid)) {
        auto ch = characteristics[cuuid];
        auto buffer = Windows::Security::Cryptography::CryptographicBuffer::
            DecodeFromBase64String(winrt::to_hstring(valueStr));
